                ...
                {monthData.map((val, m) => {
                  ...
                  return (
                    <div key={m} style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', gap:4 }}>
                      <div style={{ width:'100%', height:barH, background: isCur ? 'var(--acc)' : 'rgba(139,175,124,0.3)', borderRadius:'3px 3px 0 0', position:'relative' }}>
                        {val > 0 && isCur && (
...
export default function HabitsPage({ onAddHabit }) {
  ...
  return (
    <div className="page-root" style={{ display:'flex', flexDirection:'column', height:'100dvh', overflow:'hidden' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8, marginTop:14, marginBottom:10 }}>
            <div className="card">
              ...
              <div className="prog-track" style={{ marginTop:10 }}>
                <div className="prog-fill prog-gold" style={{ width:`${Math.min(100, maxStreak * 8)}%`, transition:'width .4s' }}/>
              </div>
            </div>
            <div className="card">
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:8 }}>
          ...
          {habits.map((h) => {
            ...
            return (
              <div key={h.id} className="card" style={{ marginBottom:8 }}>
                <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                  ...
                  <div style={{ flex:1, minWidth:0 }}>
                    ...
                    <div style={{ fontSize:11, color:'var(--txt2)' }}>
                      {h.measure_type === 'yesno' ? 'Ya / Tidak' : `${progVal} / ${h.target_value} ${h.target_unit}`}
                ...
                {isDone ? (
                  <div style={{ marginTop:10, color:'var(--acc2)', fontSize:12, display:'flex', alignItems:'center', gap:4 }}>✓ Selesai hari ini</div>
                ) : (
      ...
      {confirm && (
        <div className="modal-backdrop" onClick={() => setConfirm(null)} style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.6)', display:'flex', alignItems:'flex-end', zIndex:100 }}>
          <div className="modal-sheet" onClick={(e) => e.stopPropagation()} style={{ background:'var(--bg2)', borderRadius:'18px 18px 0 0', padding:'20px 16px 40px', width:'100%', maxWidth:430, margin:'0 auto', border:'0.5px solid var(--bdr2)' }}>
            <div style={{ textAlign:'center', padding:'8px 0 16px' }}>
              <div style={{ fontSize:15, fontWeight:500, color:'var(--txt)' }}>Hapus "{habits.find(h => h.id === confirm)?.name}"?</div>
              <div style={{ fontSize:12, color:'var(--txt2)', marginTop:4 }}>Semua data progress akan hilang</div>
            </div>
            <div style={{ display:'flex', gap:8 }}>
              <button className="btn-outline" onClick={() => setConfirm(null)} style={{ flex:1, padding:10, background:'transparent', border:'0.5px solid var(--bdr2)', borderRadius:'var(--r-md)', color:'var(--txt2)', cursor:'pointer' }}>Batal</button>
              <button onClick={() => handleDeactivate(habits.find(h => h.id === confirm))} style={{ flex:2, padding:10, background:'var(--red-dim)', color:'var(--red)', border:'0.5px solid var(--red)', borderRadius:'var(--r-md)', cursor:'pointer' }}>Ya, hapus</button>
            </div>
          </div>
        </div>
      )}
